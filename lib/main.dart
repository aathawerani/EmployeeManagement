...
class ApiWidget extends InheritedWidget {
  ...
  static ApiWidget of(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType() as ApiWidget;
  }
  ...
  Future<List<Employee>?> loadAndParseEmployees() async {
    ...
    final response = await HTTP.get(url as Uri).timeout(_TIMEOUT);
  ...
  Future<dynamic> saveEmployee(Employee employee) async {
    ...
    final response = isUpdate
        ? await HTTP
            .put(uri as Uri, body: json.encode(employee))
            .timeout(_TIMEOUT)
        : await HTTP
            .post(uri as Uri, body: json.encode(employee))
            .timeout(_TIMEOUT);
    if (response.statusCode == 200) {
  ...
  Future<dynamic> deleteEmployee(String id) async {
    ...
    final response = await HTTP.delete(uri as Uri).timeout(_TIMEOUT);
    if (response.statusCode == 200) {
      return json.decode(response.body);
    } else {
      // If that response was not OK, throw an error.
...
class _EmployeeListWidgetState extends State<EmployeeListWidget> {
  ...
  bool _refresh = true;
  List<Employee> _employees = List.empty(growable: true);
